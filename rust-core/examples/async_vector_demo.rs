...
use mindcache_core::database::{DatabaseConfig, async_db::AsyncDatabase};
#[cfg(all(feature = "async", feature = "vector-search"))]
use mindcache_core::database::vector::VectorConfig;
#[cfg(all(feature = "async", feature = "vector-search"))]
use mindcache_core::core::{MindCacheConfig, RequestValidator, BatchRequest};
#[cfg(all(feature = "async", feature = "vector-search"))]
...
async fn main() -> Result<()> {
    ...
    let memory_tasks: Vec<_> = (0..5).map(|i| {
        ...
        tokio::spawn(async move {
            ...
            metadata.insert("category".to_string(), if i % 2 == 0 { "tech" } else { "science" }.to_string());

            let memory = MemoryItem {
                user_id,
                session_id,
                content: format!("This is async demo memory #{} about {}", 
                    i, 
                    if i % 2 == 0 { "artificial intelligence and machine learning" } 
                    else { "quantum physics and space exploration" }
                ),
    ...
    let memory_ids: Vec<String> = results.into_iter()
    ...
    for (i, memory_id) in memory_ids.iter().enumerate() {
        ...
        memory_manager.store_embedding(memory_id, embedding, "demo_model_v1").await?;
    }
    ...
    let similar_memories = memory_manager.search_similar(
        user_id, 
        query_embedding, 
        "demo_model_v1", 
        Some(3)
    ...
    let hybrid_results = memory_manager.hybrid_search(
        user_id,
        text_query,
        vector_query,
        "demo_model_v1",
    ...
    for (i, result) in hybrid_results.iter().enumerate() {
        println!("  {}. Score: {:.3} (text: {:.1}, vector: {:.3})", 
            i + 1, 
    ...
    let batch_memories: Vec<MemoryItem> = (0..20).map(|i| {
        MemoryItem {
            user_id: user_id.to_string(),
            session_id: format!("batch_session_{}", i / 5), // 4 sessions with 5 memories each
            content: format!("Batch memory {} discussing various topics in technology and science", i),
    ...
    println!("✅ Batch completed: {}/{} successful, {} errors", 
        batch_response.success_count,
        batch_response.results.len(),
        batch_response.error_count
    );
    println!("   Success rate: {:.1}%", batch_response.success_rate() * 100.0);

    // Demo 5: Export with progress tracking
    println!("\n📤 Demo 5: Export with Progress Tracking");
    println!("---------------------------------------");

    let exported_memories = memory_manager.export_user_memories_with_progress(
        user_id,
        |current, total| {
            let progress = (current as f32 / total as f32) * 100.0;
            print!("\rExporting memories: {:.1}% ({}/{})", progress, current, total);
            std::io::stdout().flush().ok();
        }
    ).await?;

    println!("\n✅ Exported {} memories for user {}", exported_memories.len(), user_id);
    ...
    println!("Operations per second: {:.1} saves, {:.1} queries", 
