...
fn main() -> Result<()> {
    ...
    #[cfg(feature = "async")]
    {
        ...
        let async_result = tokio::runtime::Runtime::new()?.block_on(setup_async_test(num_operations))?;
    ...
    {
        println!("⚠️  Async features not enabled. Compile with --features async for full comparison.");
        println!("\nSync results:");
...
fn setup_sync_test(num_operations: usize) -> Result<TestResults> {
    ...
    let batch_memories: Vec<MemoryItem> = (0..num_operations / 10).map(|i| {
        MemoryItem {
...
async fn setup_async_test(num_operations: usize) -> Result<TestResults> {
    ...
    let _memory_ids: Vec<_> = futures::future::join_all(save_tasks).await.into_iter()
    ...
    let _recall_results: Vec<_> = futures::future::join_all(recall_tasks).await.into_iter()
        .collect::<Result<Vec<_>, _>>()?;
...
fn compare_results(sync_results: TestResults, async_results: TestResults, num_operations: usize) {
    ...
    if async_results.total_time_ms < sync_results.total_time_ms {
        let improvement = ((sync_results.total_time_ms - async_results.total_time_ms) as f64 / sync_results.total_time_ms as f64) * 100.0;
        println!("  - Async is {:.1}% faster overall", improvement);
    } else {
        let degradation = ((async_results.total_time_ms - sync_results.total_time_ms) as f64 / sync_results.total_time_ms as f64) * 100.0;
...
#[cfg(feature = "async")]
fn print_comparison_table(sync_results: &TestResults, async_results: &TestResults, num_operations: usize) {
    println!("┌──────────────────┬──────────────┬──────────────┬────────────────┐");
    println!("│ Operation        │ Sync (ms)    │ Async (ms)   │ Difference     │");
    println!("├──────────────────┼──────────────┼──────────────┼────────────────┤");
    
    print_table_row("Setup", sync_results.setup_time_ms, async_results.setup_time_ms);
    print_table_row("Individual Saves", sync_results.save_time_ms, async_results.save_time_ms);
    print_table_row("Recalls", sync_results.recall_time_ms, async_results.recall_time_ms);
    print_table_row("Batch Operations", sync_results.batch_time_ms, async_results.batch_time_ms);
    print_table_row("Total", sync_results.total_time_ms, async_results.total_time_ms);
    
    println!("└──────────────────┴──────────────┴──────────────┴────────────────┘");
    
    println!("\nOperations: {} saves, {} recalls, {} batch items", 
        num_operations, num_operations / 10, num_operations / 10);
}

#[cfg(feature = "async")]
fn print_table_row(operation: &str, sync_time: u128, async_time: u128) {
    let diff = if async_time < sync_time {
        format!("-{:.1}%", ((sync_time - async_time) as f64 / sync_time as f64) * 100.0)
    } else {
        format!("+{:.1}%", ((async_time - sync_time) as f64 / sync_time as f64) * 100.0)
    };
...
fn print_test_results(name: &str, results: TestResults, num_operations: usize) {
    ...
    println!("  Save {} items: {}ms ({:.1} ops/sec)", 
        num_operations, 
