...
fn setup_benchmark_env() -> (MemoryManager, SessionManager, TempDir) {
    ...
    let db_config = DatabaseConfig {
        path: temp_dir.path().join("benchmark.db").to_string_lossy().to_string(),
...
fn create_test_memory(user_id: &str, session_id: &str, content: &str, importance: f32) -> MemoryItem {
   MemoryItem {
...
fn bench_memory_save(c: &mut Criterion) {
   ...
   for importance in [0.1, 0.5, 0.9].iter() {
       let memory = create_test_memory("bench_user", "bench_session", "Benchmark content", *importance);
...
fn bench_memory_recall(c: &mut Criterion) {
   ...
   for i in 0..1000 {
       let memory = create_test_memory(
           "recall_user",
           "recall_session",
           &format!("Recall benchmark memory {} with various keywords trading stocks crypto", i),
           0.3 + (i % 7) as f32 * 0.1,
       );
   ...
   let filters = vec![
       ("no_filter", QueryFilter {
           user_id: Some("recall_user".to_string()),
           limit: Some(50),
           ..Default::default()
       }),
       ("importance_filter", QueryFilter {
           user_id: Some("recall_user".to_string()),
           min_importance: Some(0.7),
           limit: Some(50),
           ..Default::default()
       }),
       ("keyword_filter", QueryFilter {
           user_id: Some("recall_user".to_string()),
...
fn bench_full_text_search(c: &mut Criterion) {
   ...
   for i in 0..500 {
       let content = format!("{} - iteration {}", 
                           content_templates[i % content_templates.len()], i);
   ...
   for query in search_queries {
       group.bench_function(query, |b| {
           b.iter(|| {
               black_box(memory_manager.search_memories("search_user", query, Some(20), Some(0)).unwrap())
           })
       });
   }
...
fn bench_session_operations(c: &mut Criterion) {
   ...
   for i in 0..10 {
       let session_id = session_manager.create_session(
           "session_user",
           Some(format!("Benchmark Session {}", i))
       ).unwrap();
   ...
   group.bench_function("get_user_sessions", |b| {
       b.iter(|| {
           black_box(session_manager.get_user_sessions("session_user", Some(20), Some(0)).unwrap())
       })
   });
   
   // Get a session ID for summary benchmarking
   let sessions = session_manager.get_user_sessions("session_user", Some(1), Some(0)).unwrap();
   let session_id = &sessions.data[0].id;
   
   group.bench_function("generate_session_summary", |b| {
       b.iter(|| {
           black_box(session_manager.generate_session_summary(session_id).unwrap())
       })
   });
   
   group.bench_function("search_sessions", |b| {
       b.iter(|| {
           black_box(session_manager.search_sessions("session_user", vec!["trading".to_string()]).unwrap())
       })
   });
   ...
   for data_size in [100, 1000, 5000].iter() {
       ...
       for i in 0..*data_size {
           let memory = create_test_memory(
               ...
               &format!("Scaling test memory {} with content about various topics", i),
               0.3 + (i % 7) as f32 * 0.1,
           );
           memory_manager.save_memory(memory).unwrap();
       }
       ...
       group.bench_with_input(
           ...
           |b, _| {
               b.iter(|| {
                   black_box(memory_manager.search_memories("scaling_user", "memory", Some(20), Some(0)).unwrap())
               })
           },
       );
   }
