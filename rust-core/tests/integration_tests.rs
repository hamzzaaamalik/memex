...
fn test_full_memory_lifecycle() {
    ...
    let updated = env
        .memory_manager
        .update_memory(&memory_id, update)
    ...
    let updated_memory = env
        ...
        .expect("Should get updated memory");
    ...
    let not_found = env
        .memory_manager
        .get_memory(&memory_id)
        .expect("Should handle deleted memory");
...
fn test_multi_user_scenario() {
    ...
    let alice_memories = vec![
        ...
        (
            "Set stop loss for AAPL at $170. Risk management is key.",
            0.9,
        ),
    ...
    let alice_memories = env
        .memory_manager
        .recall_memories(QueryFilter {
    ...
    assert!(alice_memories
        .data
        .iter()
        .any(|m| m.content.contains("AAPL")));

    let bob_memories = env
        .memory_manager
        .recall_memories(QueryFilter {
    ...
    assert!(bob_memories
        .data
        .iter()
        .any(|m| m.content.contains("Portfolio")));

    // Test cross-user search (should return nothing)
    let cross_search = env
        .memory_manager
        .search_memories("alice", "Portfolio", Some(10), Some(0))
...
fn test_session_management_workflow() {
    ...
    for (session_id, contents) in sessions_data {
        for content in contents {
            ...
            env.memory_manager
                .save_memory(memory)
                .expect("Should save session memory");
        }
    }
...
fn test_advanced_search_and_filtering() {
    ...
    let test_data = vec![
        (
            "Apple earnings beat expectations, stock up 5%",
            0.9,
            "tech",
            "AAPL",
        ),
        (
            "Microsoft cloud revenue grows 30% year-over-year",
            0.8,
            "tech",
            "MSFT",
        ),
        ...
        (
            "Bitcoin breaks above $50k resistance level",
            0.6,
            "crypto",
            "BTC",
        ),
        ...
        (
            "Gold hits new yearly high on inflation fears",
            0.6,
            "commodities",
            "GOLD",
        ),
    ...
    for (content, importance, category, symbol) in test_data {
        ...
        let memory = MemoryItem {
            ...
            created_at: chrono::Utc::now()
                - chrono::Duration::hours(rand::random::<u32>() as i64 % 48),
            ..Default::default()
        };
        env.memory_manager
            .save_memory(memory)
            .expect("Should save test memory");
    }
    ...
    for memory in &combined_filter.data {
        ...
        assert!(
            memory.content.to_lowercase().contains("tech")
...
fn test_memory_decay_integration() {
    ...
    let decay_test_memories = vec![
        ...
        (
            "Critical trading alert: Market crash imminent",
            0.95,
            Some(48),
            false,
        ),
        ...
        (
            "Market volatility increasing, be cautious",
            0.6,
            Some(24),
            false,
        ),
        (
            "Portfolio review scheduled for next week",
            0.5,
            Some(48),
            false,
        ),
        ...
        (
            "Weather is nice, good for market sentiment",
            0.2,
            Some(2),
            true,
        ),
    ...
    for (content, importance, ttl, make_old) in decay_test_memories {
        ...
        let id = env
            .memory_manager
            .save_memory(memory)
            .expect("Should save decay test memory");
        memory_ids.push(id);
    }

    // Get initial count
    let initial_memories = env
        .memory_manager
        .recall_memories(QueryFilter {
            user_id: Some(user_id.to_string()),
            ..Default::default()
    ...
    assert!(surviving_memories
        .data
        .iter()
        .any(|m| m.content.contains("Critical")));
    assert!(surviving_memories
        .data
        .iter()
        .any(|m| m.content.contains("Emergency")));

    println!(
        "Decay results: expired={}, compressed={}, before={}, after={}",
...
fn test_statistics_and_analytics() {
    ...
    for (user_index, user) in users.iter().enumerate() {
        ...
        for session_index in 0..3 {
            ...
            for i in 0..memory_count {
                ...
                env.memory_manager
                    .save_memory(memory)
                    .expect("Should save analytics memory");
            }
        }
    }
    ...
    for user in &users {
        ...
        let user_sessions = env
            .session_manager
            .get_user_sessions(user, Some(10), Some(0))
            .expect("Should get user sessions");

        let max_memory_count = user_sessions
            .data
            .iter()
    ...
    for user in &users {
        ...
        let parsed: Vec<MemoryItem> =
            serde_json::from_str(&exported_data).expect("Exported data should be valid JSON");
...
fn test_performance_under_load() {
    ...
    for i in 0..num_saves {
        ...
        env.memory_manager
            .save_memory(memory)
            .expect("Should save performance memory");
    }
    ...
    println!(
        "Recall performance: {} recalls in {:?} ({:.2} recalls/sec)",
        num_recalls, recall_duration, recalls_per_second
    );

    assert!(
        recalls_per_second > 30.0,
        "Recall performance too slow: {:.2} recalls/sec",
        recalls_per_second
    );
    ...
    for i in 0..num_searches {
        ...
        env.memory_manager
            .search_memories(user_id, &query, Some(20), Some(0))
            .expect("Should search memories");
    }
...
fn test_error_recovery_and_resilience() {
    ...
    assert!(!env
        .memory_manager
        .update_memory("nonexistent_id", update)
        .unwrap());
    ...
    assert!(env
        .session_manager
        .generate_session_summary("nonexistent_session")
        .is_err());

    // 4. Empty/boundary condition queries
    let empty_results = env
        .memory_manager
        .recall_memories(QueryFilter {
    ...
    let unicode_id = env
        .memory_manager
        .save_memory(unicode_memory)
        .expect("Should handle Unicode");
    let unicode_retrieved = env
        .memory_manager
        .get_memory(&unicode_id)
...
fn test_end_to_end_user_workflow() {
    ...
    for content in morning_memories {
        ...
        env.memory_manager
            .save_memory(memory)
            .expect("Should save morning memory");
    }
    ...
    for (content, importance) in trading_memories {
        ...
        env.memory_manager
            .save_memory(memory)
            .expect("Should save trading memory");
    }
    ...
    let evening_memory = MemoryItem {
        user_id: user_id.to_string(),
        session_id: trading_session.clone(),
        content: "Good trading day. Up 2.3% on the day. Need to take some profits tomorrow."
            .to_string(),
        importance: 0.8,
        metadata: {
            let mut meta = HashMap::new();
    ...
    for content in strategy_memories {
        ...
        env.memory_manager
            .save_memory(memory)
            .expect("Should save strategy memory");
    }
    ...
    let trading_session_data = all_sessions
        .data
        .iter()
    ...
    let strategy_session_data = all_sessions
        .data
        .iter()
    ...
    let parsed_memories: Vec<MemoryItem> =
        serde_json::from_str(&exported_data).expect("Should parse exported data");
    ...
    println!(
        "User {} has {} memories across {} sessions",
