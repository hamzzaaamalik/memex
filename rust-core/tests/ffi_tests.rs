...
fn test_ffi_memory_operations() {
    ...
    let memory_id = memory_id_cstr
        .to_str()
        .expect("Memory ID should be valid UTF-8");
    ...
    let retrieved_json = retrieved_cstr
        .to_str()
        .expect("Retrieved JSON should be valid UTF-8");

    // Parse and verify the retrieved memory
    let retrieved_memory: serde_json::Value =
        serde_json::from_str(retrieved_json).expect("Should parse retrieved memory JSON");
...
fn test_ffi_batch_operations() {
    ...
    assert!(
        !batch_result_ptr.is_null(),
        "Batch save should return results"
    );
...
fn test_ffi_recall_and_search() {
    ...
    let test_memories = vec![
        ...
        (
            "Bitcoin price volatility creates trading opportunities",
            0.7,
        ),
    ...
    assert!(search_response["data"][0]["content"]
        .as_str()
        .unwrap()
        .contains("Apple"));
...
fn test_ffi_session_operations() {
    ...
    if !summary_result_ptr.is_null() {
        ...
        assert!(!summary_response["summary_text"]
            .as_str()
            .unwrap()
            .is_empty());
    ...
    let search_sessions_response: serde_json::Value =
        serde_json::from_str(search_sessions_json).unwrap();
...
fn test_ffi_decay_operations() {
    ...
    assert!(
        decay_response["status"].as_str().unwrap() == "completed"
...
fn test_ffi_statistics_and_analytics() {
    ...
    let session_analytics_response: serde_json::Value =
        serde_json::from_str(session_analytics_json).unwrap();
...
fn test_ffi_unicode_handling() {
    ...
    assert!(retrieved_memory["metadata"]["emoji"]
        .as_str()
        .unwrap()
        .contains("🚀"));
