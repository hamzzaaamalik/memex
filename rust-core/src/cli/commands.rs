...
use crate::cli::{InteractiveCli, format_bytes, format_duration};
...
impl InteractiveMemoryCommands {
    ...
    pub fn create_memory_wizard(manager: &MemoryManager) -> Result<()> {
        ...
        let importance = InteractiveCli::prompt_number::<f32>(
            "Importance (0.0-1.0)", 
            Some(0.5)
        )?.clamp(0.0, 1.0);
    ...
    pub fn browse_memories(manager: &MemoryManager) -> Result<()> {
        ...
        loop {
            ...
            println!("\n{}", format!("Memories {}-{} of {} (Page {}/{})", 
                    offset + 1, 
                    offset + response.data.len(),
                    response.total_count,
                    response.page + 1,
                    response.total_pages).dimmed());
            ...
            match options[selection].as_str() {
                ...
                "Search" => {
                    ...
                    if !query.is_empty() {
                        let search_response = manager.search_memories(&user_id, &query, Some(10), Some(0))?;
    ...
    fn display_memory_details(memory: &MemoryItem) {
        ...
        println!("Importance: {}", format!("{:.2}", memory.importance).bright_yellow());
        ...
        if memory.is_compressed {
            println!("Type: {} (from {} memories)", 
                    "COMPRESSED".bright_magenta(), 
                    memory.compressed_from.len());
            println!("Original IDs: {}", memory.compressed_from.join(", ").dimmed());
        }
...
impl InteractiveSessionCommands {
    ...
    pub fn browse_sessions(manager: &SessionManager) -> Result<()> {
        ...
        match selection {
            ...
            3 => {
                ...
                let delete_memories = InteractiveCli::confirm("Also delete all memories in this session?", false)?;
                
                if InteractiveCli::confirm(&format!("Are you sure you want to delete session {}?", session_id), false)? {
    ...
    fn display_session_summary(summary: &SessionSummary) {
        ...
        println!("Memory count: {}", summary.memory_count.to_string().bright_green());
...
impl InteractiveDecayCommands {
    ...
    pub fn configure_decay(engine: &mut DecayEngine) -> Result<()> {
        ...
        let threshold = InteractiveCli::prompt_number::<f32>(
            "Importance threshold (0.0-1.0)", 
            Some(0.3)
        )?.clamp(0.0, 1.0);
        
        let max_memories = InteractiveCli::prompt_number::<usize>(
    ...
    pub fn analyze_and_run_decay(engine: &DecayEngine) -> Result<()> {
        ...
        println!("  Old memories: {:.1}%", recommendations.old_memory_percentage);
        println!("  Estimated cleanup: {} memories", recommendations.estimated_cleanup_count.to_string().bright_cyan());
        ...
        for (bucket, count) in &recommendations.age_distribution {
            ...
            println!("  {:>6}: {:4} ({:5.1}%) {}", bucket, count, percentage, bar.blue());
        }
        
        // Ask if user wants to proceed
        if recommendations.estimated_cleanup_count > 0 {
            if InteractiveCli::confirm(&format!("Run decay process? (will process {} memories)", 
                                              recommendations.estimated_cleanup_count), false)? {
   ...
   fn display_decay_results(stats: &DecayStats) {
       ...
       if let Some(completed_at) = stats.completed_at {
           ...
           println!("Duration: {}", format_duration(duration.num_seconds()).bright_cyan());
       }
       ...
       let summary_data = vec![
           ...
           ("Storage saved", format_bytes(stats.storage_saved_bytes as u64)),
       ];
           ...
           4 => {
               if InteractiveCli::confirm("This will run all optimizations. Continue?", false)? {
                   ...
                   for (i, step) in steps.iter().enumerate() {
                       ...
                       std::thread::sleep(std::time::Duration::from_millis(1000)); // Simulate work
                   }
       ...
       match selection {
           0 => {
               let backup_path = InteractiveCli::prompt_text(
                   "Backup file path", 
                   Some(&format!("mindcache_backup_{}.db", 
           ...
           1 => {
               ...
               if InteractiveCli::confirm("This will overwrite the current database. Continue?", false)? {
...
impl SystemDiagnostics {
   ...
   pub fn run_full_diagnostics(database: &Database) -> Result<()> {
       ...
       if issues.is_empty() && warnings.is_empty() {
           println!("{}", "🎉 All diagnostics passed! System is healthy.".bright_green().bold());
       } else {
           if !issues.is_empty() {
   ...
   pub fn performance_monitor() -> Result<()> {
       ...
       for (metric, _type, value, status) in metrics {
           ...
           println!("  {:<15}: {:>12} {}", metric, value.bright_blue(), status_color);
       }
       ...
       // Simple ASCII graph
       let hours = 24;
       let data = (0..hours).map(|_| fastrand::u32(50..100)).collect::<Vec<_>>();
