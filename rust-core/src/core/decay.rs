...
impl DecayEngine {
    ...
    pub fn update_policy(&mut self, policy: DecayPolicy) -> Result<()> {
        ...
        self.policy = policy;
        log::info!("Updated decay policy: max_age={}h, threshold={}, compression={}", 
    ...
    pub fn run_decay(&self) -> Result<DecayStats> {
        ...
        log::info!("Decay process completed in {}ms (run_id: {})", 
                  duration.num_milliseconds(), run_id);
        log::info!("Results: expired={}, compressed={}, sessions={}, before={}, after={}", 
                  stats.memories_expired, stats.memories_compressed, stats.sessions_summarized,
                  stats.total_memories_before, stats.total_memories_after);
    ...
    fn expire_old_memories(&self) -> Result<usize> {
        ...
        let expired_count = self.database.cleanup_expired()
            ...
            if memory.importance < self.policy.importance_threshold {
                match self.database.delete_memory(&memory.id) {
                    Ok(true) => {
                        ...
                        log::debug!("Expired old memory {} (age: {}h, importance: {})", 
                                  memory.id, 
    ...
    fn compress_old_memories(&self) -> Result<usize> {
        let cutoff_date = Utc::now() - chrono::Duration::hours(self.policy.max_age_hours as i64 / 2);
        ...
        let old_memories_response = self.database.recall_memories(&filter)?;
        let old_memories: Vec<_> = old_memories_response.data.into_iter()
            ...
            if memories.len() >= 3 {
                match self.create_compressed_memory(memories) {
                    Ok(compressed_memory) => {
                        ...
                        log::debug!("Compressed {} memories from session {} into {}", 
                                  compressed_memory.compressed_from.len(), session_id, compressed_id);
                    }
                    Err(e) => {
                        log::error!("Failed to compress memories for session {}: {}", session_id, e);
                    }
                }
            }
        }
    ...
    fn create_compressed_memory(&self, memories: Vec<MemoryItem>) -> Result<MemoryItem> {
        ...
        let compressed_memory = MemoryItem {
            ...
            metadata: {
                ...
                metadata.insert("original_count".to_string(), sorted_memories.len().to_string());
                metadata.insert("key_points".to_string(), serde_json::to_string(&key_points).unwrap_or_default());
                metadata.insert("date_range_start".to_string(), sorted_memories.first().unwrap().created_at.to_rfc3339());
                metadata.insert("date_range_end".to_string(), sorted_memories.last().unwrap().created_at.to_rfc3339());
                metadata
            },
    ...
    fn generate_compression_summary(&self, memories: &[MemoryItem]) -> Result<String> {
        ...
        let sample_content: String = sample_memories.iter()
            .take(3)
            .map(|m| {
                if m.content.len() > 50 {
                    format!("{}...", &m.content[..50])
    ...
    fn extract_key_points(&self, memories: &[MemoryItem]) -> Vec<String> {
        ...
        for memory in memories {
            let words: Vec<String> = memory.content.to_lowercase()
                .split_whitespace()
                .filter_map(|word| {
    ...
    fn summarize_old_sessions(&self) -> Result<usize> {
        ...
        for (session_id, memory_count) in session_memory_counts {
            if memory_count >= 5 {
                ...
                log::debug!("Would summarize session {} with {} memories", session_id, memory_count);
                summarized_count += 1;
            }
        }
    ...
    pub fn get_decay_recommendations(&self) -> Result<DecayRecommendations> {
        ...
        let old_memories = *age_distribution.get("1y+").unwrap_or(&0) + *age_distribution.get("3m-1y").unwrap_or(&0);
        ...
        if old_percentage > 50.0 {
            recommendations.push("Consider running decay process - over 50% of memories are old".to_string());
        }
        
        if total_memories > self.policy.max_memories_per_user {
            recommendations.push("Memory count exceeds configured limits - cleanup recommended".to_string());
        }
        
        if *age_distribution.get("0-24h").unwrap_or(&0) > (total_memories / 2) {
            recommendations.push("High recent activity detected - consider adjusting TTL policies".to_string());
        }
...
mod tests {
    ...
    fn test_key_points_extraction() {
        ...
        assert!(key_points.contains(&"trading".to_string()) || 
