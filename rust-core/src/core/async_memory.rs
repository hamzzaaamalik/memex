...
impl AsyncSessionManager {
    ...
    pub async fn create_session(&self, user_id: &str, name: Option<String>) -> Result<String> {
        ...
        let session_id = self.database.create_session(user_id.to_string(), name).await
    ...
    pub async fn get_user_sessions(&self, user_id: &str, limit: Option<usize>, offset: Option<usize>) -> Result<PaginatedResponse<Session>> {
        let start = std::time::Instant::now();
        
        // Rate limiting
        self.validator.validate_request(1)?;
        
        let response = self.database.get_user_sessions(user_id.to_string(), limit, offset).await
    ...
    async fn generate_intelligent_summary(&self, memories: &[MemoryItem]) -> Result<SessionSummary> {
        if memories.is_empty() {
    ...
    fn generate_summary_text_sync(memories: &[MemoryItem], key_topics: &[String]) -> Result<String> {
    ...
    pub async fn search_sessions(&self, user_id: &str, keywords: Vec<String>) -> Result<Vec<Session>> {
        // Rate limiting
        ...
        if delete_memories {
            ...
            for memory in memories_response.data {
                ...
                delete_tasks.push(async move {
                    database.delete_memory(memory_id).await
                });
            }
            ...
            log::info!("Deleted {} memories from session {}", deleted_count, session_id);
        }
