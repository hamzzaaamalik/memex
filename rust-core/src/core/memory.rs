...
use crate::core::{BatchRequest, BatchResponse, RequestValidator, PerformanceMonitor};
...
impl MemoryManager {
    ...
    pub fn save_memory(&self, mut memory: MemoryItem) -> Result<String> {
        ...
        // Save to database
        let result = self.database.save_memory(&memory)
    ...
    pub fn save_memories_batch(&self, request: BatchRequest<MemoryItem>) -> Result<BatchResponse<String>> {
        // Validate batch size
        ...
        log::info!("Batch save completed: {}/{} successful", 
                  response.success_count, response.results.len());
    ...
    pub fn recall_memories(&self, filter: QueryFilter) -> Result<PaginatedResponse<MemoryItem>> {
        ...
        // Execute query
        let result = self.database.recall_memories(&filter)
        ...
        match &result {
            Ok(response) => {
                log::debug!("Recalled {} memories in {}ms (page {}/{})", 
                           response.data.len(), duration, response.page + 1, response.total_pages);
            }
            Err(e) => {
    ...
    pub fn get_memory(&self, id: &str) -> Result<Option<MemoryItem>> {
        ...
        let result = self.database.get_memory(id)
    ...
    pub fn update_memory(&self, id: &str, updates: MemoryUpdate) -> Result<bool> {
        ...
        if let Some(ttl_hours) = updates.ttl_hours {
            ...
            memory.expires_at = ttl_hours.map(|ttl| {
                memory.created_at + chrono::Duration::hours(ttl as i64)
            });
        }
    ...
    pub fn delete_memory(&self, id: &str) -> Result<bool> {
        ...
        let result = self.database.delete_memory(id)
    ...
    pub fn search_memories(&self, user_id: &str, query: &str, limit: Option<usize>, offset: Option<usize>) -> Result<PaginatedResponse<MemoryItem>> {
    ...
    pub fn get_session_memories(&self, user_id: &str, session_id: &str, limit: Option<usize>, offset: Option<usize>) -> Result<PaginatedResponse<MemoryItem>> {
    ...
    pub fn get_important_memories(&self, user_id: &str, threshold: f32, limit: Option<usize>) -> Result<PaginatedResponse<MemoryItem>> {
    ...
    pub fn get_memories_in_range(&self, user_id: &str, from: DateTime<Utc>, to: DateTime<Utc>, limit: Option<usize>) -> Result<PaginatedResponse<MemoryItem>> {
    ...
    pub fn export_user_memories(&self, user_id: &str) -> Result<Vec<MemoryItem>> {
        ...
        log::info!("Exported {} memories for user {}", all_memories.len(), user_id);
        Ok(all_memories)
    }
    ...
    pub fn get_user_memory_stats(&self, user_id: &str) -> Result<UserMemoryStats> {
        ...
        Ok(UserMemoryStats {
            ...
            oldest_memory: all_memories.iter().min_by_key(|m| m.created_at).map(|m| m.created_at),
            newest_memory: all_memories.iter().max_by_key(|m| m.created_at).map(|m| m.created_at),
        })
    }
...
mod tests {
   ...
   fn test_search_memories() {
       ...
       // Search for "AAPL"
       let response = manager.search_memories("test_user", "AAPL", Some(10), Some(0)).unwrap();
       assert_eq!(response.data.len(), 1);
       assert!(response.data[0].content.contains("AAPL"));
       
       // Search for "price"
       let response = manager.search_memories("test_user", "price", Some(10), Some(0)).unwrap();
       assert_eq!(response.data.len(), 1);
